...
int main(int argc, char* argv[]) 
{
	...
	{ // Generate key file
		...
		uint8_t* des_key = (uint8_t*) malloc(8 * sizeof(uint8_t));
		...
		fclose(key_file);
		     ...
		     (strcmp(argv[1], ACTION_DECRYPT) == 0)) 
	{ // Encrypt or decrypt
		if (argc != 5) 
		{
			printf("Invalid # of parameters (%d) specified."
				   "Usage: run_des [-e|-d] keyfile.key input.file output.file", argc);
		...
		bytes_read = fread(des_key, sizeof(uint8_t), DES_KEY_SIZE, key_file);
		...
		uint16_t bytes_written, process_mode;
		uint64_t block_count = 0, number_of_blocks;

		uint8_t* data_block      = (uint8_t*) malloc( 8 * sizeof(uint8_t));
		uint8_t* processed_block = (uint8_t*) malloc( 8 * sizeof(uint8_t));
